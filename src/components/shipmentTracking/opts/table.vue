 <template>
  <div>
    ...
    <el-table :data="tableData.slice((pagination.currpage - 1) * pagination.pagesize, pagination.currpage * pagination.pagesize)" :header-cell-style="getRowClass" :row-class-name="tableRowClassName" v-loading="loading" border style="width: 100%">
      ...
      <el-table-column label="货件名">
        <template slot-scope="scope">
          <div @click="findItemByShipmentId(scope.row)" class="shipmentTitle"> {{scope.row.shipingName}}</div>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getShipmentByParam(){
      ...
      this.$ajax({
        method: 'post',
        url:this.URL_ROOT + this.ORDERANDOVERSEA_SERVICE +'/queryShipmentMsg/getShipmentByParam',
        data: data
      }).then(res => {
        console.log(res)
    ...
    findItemByShipmentId(item){
        ...
        this.$ajax({
          method: 'post',
          url: this.URL_ROOT + this.ORDERANDOVERSEA_SERVICE +'/queryShipmentMsg/findItemByShipmentId',
          data: {
