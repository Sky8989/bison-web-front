<template>
  <el-form :model="ruleForm" ref="ruleForm" label-width="80px">
    ...
    <div>
      ...
      <el-form-item label="产品型号" prop="productCodes">
        <el-select
          v-model="ruleForm.productCodes"
          multiple
          collapse-tags
          filterable
          placeholder="请选择产品型号"
        >
          <div class="businessReport-customize-select-btn-group">
            <el-button size="mini" @click="selectAll(arrData.FindProductModel,'productCodes')">全部选中</el-button>
            <el-button size="mini" @click="selectDel('productCodes')">全部删除</el-button>
          </div>
          <div class="businessReport-customize-select-height"></div>
          <el-option
      ...
      <download :downloadForm="downloadForm" url="winitSearch" :tHeader="tHeader" :filterVal="filterVal"></download>
    </div>
  </el-form> 
</template> 
<script>
...
export default {
  ...
  methods: {
    ...
    getTimePoints(time) {
      ...
      this.$ajax({
        method: "post",
        url: this.URL_ROOT + this.OVERSEASWAREHOUSESTORAGET_SERVICE + "/winitSearch/findByTime?timestamp=" + time
      }).then(res => {
    ...
    getFindCountry() {
      this.$ajax
        .post(this.URL_ROOT + this.OVERSEASWAREHOUSESTORAGET_SERVICE + "/winitSearch/findCountry")
        .then(res => {
          if (res.data.code === ERR_OK) {
    ...
    getFindWarehouse() {
      this.$ajax
        .post(
          this.URL_ROOT + this.OVERSEASWAREHOUSESTORAGET_SERVICE + "/winitSearch/findWarehouseByCountry?countryIds=" +
        ...
        .then(res => {
          if (res.data.code === ERR_OK) {
            ...
            if(this.arrData.findWarehouse){
              this.ruleForm.warehouseIds = this.ruleForm.warehouseIds.filter(item => !this.arrData.findWarehouse.every(ele => ele.warehouseId !== item));
            }else{
      ...
      this.$ajax
        ...
        .then(res => {
          if (res.data.code && res.data.data) {
            ...
            this.ruleForm.categoryIds = this.ruleForm.categoryIds.filter(item => !this.arrData.findCategorys.every(ele => ele.productCategoryId !== item));
          }else{
      ...
      this.$ajax
        ...
        .then(res => {
          if (res.data.code === ERR_OK && res.data.data) {
            ...
            this.ruleForm.productCodes = this.ruleForm.productCodes.filter(item => !this.arrData.FindProductModel.every(ele => ele.productId !== item));
          }else{
            this.ruleForm.productCodes = []
          }
        });
    },
    // 全部选中
