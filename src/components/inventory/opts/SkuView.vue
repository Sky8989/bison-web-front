<template>
  ...
  <div name="SkuView">
        ...
        <el-row :gutter="10">
          ...
          <el-col :span="3">
            <el-select placeholder="BU" filterable clearable v-model="searchInfo.businessUnitId" @change="chooseBU"
                       size="mini">
              <el-option v-for="bu in BUList" :key="bu.businessUnitId" :value="bu.businessUnitId"
                         :label="bu.businessUnit"></el-option>
            </el-select>
          </el-col>

          <el-col :span="3">
            <el-select placeholder="产品系列" filterable clearable v-model="searchInfo.productCategoryId"
                       @change="chooseProductCategory" size="mini">
              <el-option v-for="productCategory in productCategoryList" :key="productCategory.productCategoryId"
                         :value="productCategory.productCategoryId"
                         :label="productCategory.productCategory"></el-option>

            </el-select>
          </el-col>

          <el-col :span="3">
            <el-select placeholder="账号" filterable clearable v-model="searchInfo.brandId" size="mini"
          ...
          <el-col :span="3">
            <el-select placeholder="国家" filterable clearable v-model="searchInfo.countryId" @change="chooseCountry"
                       size="mini">
              <el-option v-for="country in countryList" :key="country.countryId" :value="country.countryId"
          ...
          <el-col :span="4">
            <el-select placeholder="产品型号" filterable clearable v-model="searchInfo.productId" @change="chooseProduct"
                       size="mini">
              <el-option v-for="product in productList" :key="product.productId" :value="product.productId"
                         :label="product.productModelNumber"></el-option>
            </el-select>
          </el-col>

          <el-col :span="4">
            <el-select placeholder="SKU" filterable clearable v-model="searchInfo.sellerSkuId" size="mini">
              <el-option v-for="sellerSku in sellerSkuList" :key="sellerSku.sellerSkuId" :value="sellerSku.sellerSkuId"
                         :label="sellerSku.sku"></el-option>
            </el-select>
          </el-col>
    ...
    <!--表格-->
    <el-table name="showTable" border="border" height="350px" style="width: 100%" :data="InventoryList"
      ...
      <el-table-column prop="sku" label="是否在售">
        <template slot-scope="scope">
          <span v-if="scope.row.yFlag == 1">
            在售
          </span>
          <span v-if="scope.row.yFlag == 0">
           未售
          </span>
        </template>
      </el-table-column>
            ...
            <el-popover trigger="hover" placement="top">
              <p>
                <span v-for="(itemKeyObj,index) in scope.row.lastItemKeyList" :key="itemKeyObj.itemKeyId">
                  {{itemKeyObj.itemKey}}{{itemKeyNameLast}} <span
                  v-if="index < scope.row.lastItemKeyList.length-1 ">+</span></span>
              </p>
            ...
            <el-popover trigger="hover" placement="top">
              <p>
                <span v-for="(itemKeyObj,index) in scope.row.estItemKeyList" :key="itemKeyObj.itemKeyId">
                  {{itemKeyObj.itemKey}}{{itemKeyNameEst}} <span
                  v-if="index < scope.row.estItemKeyList.length-1 ">+</span></span>
              </p>
  ...
  export default {
    ...
    methods: {
      init() {
        ...
        this.initBrandList()
        this.initSellSkuList()
        this.search()
      },
      initBU() { //所有BU
        this.$ajax.get(this.URL_ROOT + this.URL_PREFIX + "/businessUnit/findAllBU", {}).then(res => {
      ...
      initSellSkuList() {
        var url =this.URL_ROOT + this.PREFIX_INVENTORY + "/inventoryBySKUHandler/findSellSkuList"
        this.$ajax.get(url, {}).then(res => {
          if (res.data.code == "200") {
            var data = res.data.data
            this.sellerSkuList = data
          }
        })
      ...
      chooseProduct() {
        ...
        var productId = this.searchInfo.productId

        if (productId != null && productId != 0 && productId != '') {

          var url =this.URL_ROOT + this.PREFIX_INVENTORY + "/inventoryBySKUHandler/findSellerSkuListByProductId/" + productId
          console.log(url)
          ...
          this.$ajax.get(url, {}).then(res => {
            if (res.data.code == "200") {
              ...
              this.sellerSkuList = []
              this.sellerSkuList = res.data.data
            }
          }).then(error => {
            console.log(error)
          })
      ...
      search() {
        var url =this.URL_ROOT + this.PREFIX_INVENTORY + "/inventoryBySKUHandler/findSkuInventoryList"
        ...
        var data = {
          ...
          sellerSkuId: this.searchInfo.sellerSkuId
        }
