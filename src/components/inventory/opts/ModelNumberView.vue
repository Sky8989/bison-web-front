<template>
  <div name="ModelNumberView">
    ...
    <el-button type="primary" @click="dialogVisible = true" size="mini">设置</el-button>
    ...
    <el-dialog
      ...
      <el-form  :model="formInfo" label-width="148px">
        ...
        <el-form-item label="备货日均" :rules="[{required: true, message: '备货日均不能为空'}]">
          ...
          <el-input-number  :step="0.1" :precision="2" :max="1.0" :min="0.0" size="mini"
                           v-model="formInfo.lastUnitsAvgDayRatio" controls-position="right"></el-input-number> 历史日均比率
          　
          <el-input-number  :step="0.1" :precision="2" :max="1.0" :min="0.0"  size="mini"
                           v-model="formInfo.estUnitsAvgDayRatio" controls-position="right"></el-input-number> 预测日均比率
        </el-form-item>
        ...
        <el-form-item label="提醒设置一" :rules="[{required: true, message: '提醒设置一不能为空'}]">
          ...
          近 <el-input-number :step="1"  :max="1000" :min="0"  size="mini" v-model="formInfo.bigLastUnitsAvgDay" controls-position="right"></el-input-number> 天日均 >=

          <el-select size="mini" v-model="formInfo.bigItemKeyId" >
            <el-option v-for="itemKey in estItemKeyList" :key="itemKey.itemKeyId" :value="itemKey.itemKeyId" :label="itemKey.itemKey+ itemKeyNameEst"></el-option>
          </el-select> <el-input-number  :step="0.1" :precision="2" :max="1.0" :min="0.0"  size="mini"  placeholder="填写比率" v-model="formInfo.bigEstUnitisAvgDayRatio" controls-position="right"></el-input-number> 预测数量

        </el-form-item>

        <el-form-item label="提醒设置二" :rules="[{required: true, message: '提醒设置二不能为空'}]">

          近 <el-input-number :step="1"  :max="1000" :min="0"  size="mini" v-model="formInfo.samllLastUnitsAvgDay" controls-position="right"></el-input-number> 天日均 <=
          <el-select size="mini" v-model="formInfo.samllItemKeyId"  >
             <el-option v-for="itemKey in estItemKeyList" :key="itemKey.itemKeyId" :value="itemKey.itemKeyId" :label="itemKey.itemKey+ itemKeyNameEst"></el-option>
          </el-select>  <el-input-number  :step="0.1" :precision="2" :max="1.0" :min="0.0"  size="mini"  placeholder="填写比率" v-model="formInfo.samllEstUnitsAvgDayRatio" controls-position="right"></el-input-number> 预测数量
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary"  size="mini" @click="saveFromInfo" v-if="formInfo.productId != null && formInfo.productId != 0">保 存</el-button>
    ...
    export default {
      ...
      methods:{
        ...
        checkBu(){
          ...
          var url = this.HOST  + "/businessUnitHandler/findByUserId/" + this.userId
          this.$ajax.get(url,{
          }).then(res => {
        ...
        search(){
          var url  = this.HOST +"/inventoryByModelNumberHandler/findModelNumberInventoryList"
        ...
        getItemKeyList(productId){
          var url = this.HOST +"/itemKeyHandler/findItemKeyByProductId/" + productId
          this.$ajax.get(url,{
          }).then(res => {
            if(res.data.code == 200){
              console.log(res.data.data)
        ...
        //当前产品存在数据 填充数据
          var url = this.HOST +"/inventoryByModelNumberHandler/findProductInventoryInfo/" + productId
          this.$ajax.get(url,{
          }).then(res => {
            if(res.data.code == 200){
              console.log(res.data.data)
        ...
        saveALlProductInventory(){
          this.formInfo.userId = this.userId
          var url = this.HOST +"/inventoryByModelNumberHandler/saveAllProductInventoryInfo"
          this.$ajax({
            method: 'post',
            url: url,
            data: this.formInfo
        ...
        saveOneRowProductInventory(){
          ...
          var url = this.HOST +"/inventoryByModelNumberHandler/saveProductInventoryInfo"
          this.$ajax({
            method: 'post',
            url: url,
            data: this.formInfo
